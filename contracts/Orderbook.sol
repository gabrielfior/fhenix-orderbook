...
import { euint32,inEuint32,euint8, ebool, FHE } from "@fhenixprotocol/contracts/FHE.sol";
...
contract Orderbook is IOrderbook, ReentrancyGuard {
    ...
    // ToDo - Use inEuint, since that's output from JS encrypt
    function placeBuyOrder (
        inEuint32 calldata price,
        inEuint32 calldata amountOfBaseToken
    ) external nonReentrant {
        // for (uint32 i = 0; i < list_size; i++ ) {
        //     bool something = (price.lt(sells[i].price) && amountOfBaseToken.lt(sells[i].amount)).decrypt();
        //     if (seomthig) {
        //         // eurica
        //     } 
        // }
        ...
        // uint32 amountReflect = FHE.decrypt(FHE.asEuint32(amountOfBaseToken));
